<?php
	...
	class saleController extends mainModel{
			...
			if($producto==""){
				...
				<article class="message is-warning mt-4 mb-4">
				    ...
				    <div class="message-body has-text-centered">
						...
						Debes de introducir el nombre, código o palabra clave de un producto
				    </div>
				</article>';
				exit();
            }
            ...
            if($datos_productos->rowCount()>=1){
				...
				foreach($datos_productos as $rows){
					...
					<tr class="has-text-left" >
                        <td><i class="fas fa-box fa-fw"></i> &nbsp; '.$rows['producto_nombre'].' Costo del producto: $'.$rows['producto_precio_venta'].'</td>
		...
		public function listarVentaControlador($pagina,$registros,$url,$busqueda){
			...
			$campos_tablas="venta.venta_id,venta.venta_codigo,venta.venta_fecha,venta.venta_hora,venta.venta_total,venta.usuario_id,venta.caja_id,usuario.usuario_id,usuario.usuario_nombre,usuario.usuario_apellido";

			if(isset($busqueda) && $busqueda!=""){

				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE (venta.venta_codigo='$busqueda') ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";
